...
public final class NodeLocators {
	 ...
	 *            -- key的hash值产生算法
	 * @param nodes
	 *            -- 一致性hash环上的节点集合. map的key的值为节点在环上的位置
	 * @return -- 节点定位器
	 */
	...
	public static class ConsistentHashLocator<T> implements NodeLocator<T> {
		 ...
		 * 定位节点
		 * 
		 * @see cn.tianya.fw.core.vt.locate.NodeLocator#locate(java.lang.String,
		 * int)
		 */
		public T locate(String key, int strategyCode) {
			long h = ha.hash(key);
		 ...
		 * 
		 * @see
		 * cn.tianya.fw.core.vt.locate.NodeLocator#setHashAlgorithm(cn.tianya
		 * .fw.core.vt.locate.HashAlgorithm)
		 */
		public void setHashAlgorithm(HashAlgorithm hashStrategy) {
			this.ha = hashStrategy;
		}
		...
		/**
		 * 引用一个节点编号获取下一个获选编号 备注: 用于分摊一个引用节点的请求压力时使用
		 * 
		 * @param quote
		 *            -- 引用节点
		 * @return -- 候选节点编号
		...
		private void checkHashRange(long hash) {
			...
			if (hash > 4294967295L) {
				throw new IllegalArgumentException("一致性hash的hash值范围[0, 2^32)=[0, 4294967295], 不能存在大于4294967295的节点:"
						+ hash);
			}
		}
	...
	public static class KetamaConsistentHashLocator<T> implements NodeLocator<T> {
		 ...
		 * 
		 * @see
		 * cn.tianya.fw.support.config.locate.NodeLocator#setNodes(java.util
		 * .Map)
		 */
		public void setNodes(Map<Long, T> ns) {
			for (T item : ns.values()) {
		 ...
		 * 
		 * @see
		 * cn.tianya.fw.support.config.locate.NodeLocator#setHashAlgorithm(cn
		 * .tianya.fw.support.config.locate.HashAlgorithm)
		 */
		public void setHashAlgorithm(HashAlgorithm hash) {
			this.ha = hash;
		}
		 ...
		 * 
		 * @see
		 * cn.tianya.fw.support.config.locate.NodeLocator#getHashAlgorithm()
		 */
		public HashAlgorithm getHashAlgorithm() {
			return ha;
		}
		 ...
		 * 
		 * @see
		 * cn.tianya.fw.support.config.locate.NodeLocator#locate(java.lang.String
		 * , int)
		 */
		public T locate(String key, int strategyCode) {
			if (circle.isEmpty()) {
